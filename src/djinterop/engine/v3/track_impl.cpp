    ...
    const track_snapshot& snapshot, const information_row& information)
{
    ...
    return track_row{
        ...
        .play_order = djinterop::util::optional_static_cast<int64_t>(
...
track_snapshot track_impl::snapshot() const
{
    ...
    const auto performance_data_row_maybe =
        library_->performance_data().get(id());
...
void track_impl::update(const track_snapshot& snapshot)
{
    ...
    library_->track().update(track_row);

    const auto performance_data_row =
        snapshot_to_performance_data_row(track_row.id, snapshot);
    library_->performance_data().update(performance_data_row);
}
...
{
    auto track_data = library_->performance_data().get_track_data(id());
...
void track_impl::set_average_loudness(std::optional<double> average_loudness)
{
    ...
    library_->performance_data().set_track_data(id(), track_data);
}

std::vector<beatgrid_marker> track_impl::beatgrid()
{
    auto beat_data = library_->performance_data().get_beat_data(id());
...
void track_impl::set_beatgrid(std::vector<beatgrid_marker> beatgrid)
{
    ...
    library_->performance_data().set_beat_data(id(), beat_data);
}
...
{
    auto quick_cues = library_->performance_data().get_quick_cues(id());
...
void track_impl::set_hot_cue_at(int index, std::optional<hot_cue> cue)
{
    ...
    library_->performance_data().set_quick_cues(id(), quick_cues);
}
...
{
    auto loops = library_->performance_data().get_loops(id());
...
void track_impl::set_loop_at(int index, std::optional<loop> l)
{
    ...
    library_->performance_data().set_loops(id(), loops);
}

std::vector<std::optional<loop>> track_impl::loops()
{
    return convert::read::loops(library_->performance_data().get_loops(id()));
}

void track_impl::set_loops(std::vector<std::optional<loop>> loops)
{
    auto converted = convert::write::loops(loops);
    library_->performance_data().set_loops(id(), converted);
}
...
std::vector<waveform_entry> track_impl::waveform()
{
    ...
    auto overview_waveform_data =
        library_->performance_data().get_overview_waveform_data(id());
...
void track_impl::set_waveform(std::vector<waveform_entry> waveform)
{
    ...
    library_->performance_data().set_overview_waveform_data(
        id(), overview_waveform_data);
}
    ...
    const track_snapshot& snapshot)
{
    ...
    auto performance_data_row = snapshot_to_performance_data_row(id, snapshot);
